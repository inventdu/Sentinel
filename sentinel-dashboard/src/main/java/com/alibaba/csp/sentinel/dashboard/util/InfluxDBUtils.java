...
public class InfluxDBUtils {
    ...
    @Value("${sentinel.dashboard.influxdb.url}")
    public void setUrl(String url) {
        InfluxDBUtils.url = ConfigUtil.getConfig("sentinel.dashboard.influxdb.url", url);
    }

    @Value("${sentinel.dashboard.influxdb.username}")
    public void setUsername(String username) {
        InfluxDBUtils.username = ConfigUtil.getConfig("sentinel.dashboard.influxdb.username", username);
    }

    @Value("${sentinel.dashboard.influxdb.password}")
    public void setPassword(String password) {
        InfluxDBUtils.password = ConfigUtil.getConfig("sentinel.dashboard.influxdb.password", password);
    }
    ...
    public static <T> List<T> queryList(String database, String sql, Map<String, Object> paramMap, Class<T> clasz) {
        QueryResult queryResult = query(database, (database1, influxDB) -> {
            BoundParameterQuery.QueryBuilder queryBuilder = BoundParameterQuery.QueryBuilder.newQuery(sql);
            queryBuilder.forDatabase(database1);
